...
use rdkafka2::{
    ...
    topic::{NativeTopic, Partitioner, Topic},
};
...
fn create_records<F>(topic: &Topic<F>, count: usize) -> Vec<OwnedBaseRecord>
where
    F: Partitioner,
{
    (0..count)
...
fn custom_partitioner_to_2<'a>(_: &'a [u8], _: i32) -> i32 {
    2
}
...
async fn custom_partitioner_production(config: ClientConfig, topic_name: String) {
    ...
    let (producer, delivery_stream) = test_producer(config);
    let mut topic = Topic::builder()
        .name(topic_name.as_str())
        .partitioner(|_, _| 2)
        .build();
    let topic_reg = producer
        .register_topic(&mut topic)
        .expect("topic to be registered");
    drop(topic);
    drop(producer);

    let name = topic_reg.name();
